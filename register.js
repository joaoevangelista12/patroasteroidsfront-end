function register() {
    ...
    let usuarioExistente = listerUser.some(user => user.usuarioCad === usuario.value);
    ...
    if (!regex.test(usuario.value)) {
        ...
        msgError.innerHTML = '<strong>Nome de usuário não pode conter acentos ou espaços!</strong>';
        return;
    }
...
function toggleBotoesDesabilitados() {
    ...
    form.buttonRegister().disabled = !(emailValido && passwordValido && confirmPasswordValido && nomeValido);
}
